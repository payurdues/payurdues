                ...
                <div class="dashboard-content">
                    ...
                    <div class="dashboard-content_details mb-5 col">
                        ...
                        <div class="countdown_card my-4">
                            <div class="d-md-flex text-center justify-content-center align-items-center gap-3 col-md-8 col-lg-auto">
                        ...
                        <div class="text-center my-5">
                            <h4>Here is the Final Voting Result</h4>
                        </div>

                        <div class="row row-cols-1 row-cols-md-2 justify-content-between g-3">
                            @foreach ($election->categories as $category)
                                <div class="col">
                                    <div class="accordion" id="categoryAccordion{{ $category->id }}">
                                        <div class="accordion-item">
                                            <div class="accordion-header">
                                                <button class="accordion-button py-3 collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#category{{ $category->id }}" aria-expanded="false" aria-controls="category{{ $category->id }}">
                                                    <h4 class="text-color-gradient">{{ $category->name }}</h4>
                                                </button>
                                            </div>

                                            <div id="category{{ $category->id }}" class="accordion-collapse collapse p-3 p-md-4 bg-white m-2 mt-0" data-bs-parent="#categoryAccordion{{ $category->id }}">
                                                <div class="accordion-body">
                                                    <div class="row row-cols-2 g-2 candidate-group" data-position="{{ $category->name }}">
                                                        @foreach ($results[$category->id] as $entry)
                                                            @php
                                                                $candidate = $entry['candidate'];
                                                                $votes = $entry['votes'];
                                                                $isWinner = $entry['is_winner'];
                                                            @endphp
                                                            <div class="col candidate-option {{ $isWinner ? 'selected' : '' }}">
                                                                <div class="bg-white mt-3 p-0 candidate {{ $isWinner ? 'winner' : '' }}">
                                                                    <h5 class="text-center">{{ $votes }} votes</h5>
                                                                    <div class="square-image-container position-relative">
                                                                        <img src="{{ asset('storage/' . $candidate->image) }}" alt="{{ $candidate->full_name }}">
                                                                        @if ($isWinner)
                                                                            <div class="bg-gradient p-2 text-white position-absolute bottom-0 w-100 text-center">
                                                                                <p class="fw-bold m-0">WINNER</p>
                                                                            </div>
                                                                        @endif
                                                                    </div>
                                                                    <div class="p-4 py-0 text-center candidate-details">
                        ...
                        <div class="row row-cols-md-2 g-3 mt-4 justify-content-center">
                            <a href="#" class="btn btn-pay-gradient modal-button">View Result</a>
                        </div>
                    </div>
